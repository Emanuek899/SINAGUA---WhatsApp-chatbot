...
require_once __DIR__ . "/../models/DebtsModel.php";
require_once __DIR__ . "/../models/PaymentsModel.php";
require_once __DIR__ . "/../models/AccountNumbersModel.php";
...
function generarPdf($accountNum, $paymentId) {
    // Configuración inicial (modelos y datos)
    //ngrok port 80
    $BASE_URL = $_ENV["REPORT_URL"];
    $accountModel = new AccountNumbersModel();
    $usersModel = new UsersModel();
    $paymentsModel = new PaymentsModel();
    $debtsModel = new DebtsModel();

    $paymentObject = $paymentsModel->getById($paymentId);
    if($paymentObject["success"] === false){
        return "pago no encontrado";
    }

    $accountId = $accountModel->getByNum($accountNum);
    if (!$accountId || !$accountId['success']) {
        return "Cuenta no encontrada";
    }
    $accountIdInt = (int)$accountId["data"]["id"];

    $user = $usersModel->getByAccount($accountIdInt);
    if (!$user) {
        return "Usuario no encontrado";
    }

    $debtId = $paymentObject["data"]["debt_id"];
    $debt = $debtsModel->getById($debtId);
    if (!$debt || !$debt['success']) {
        return ["error " => "Deuda no encontrada", "debt_id" => $debtId];
    }

    $deuda = $debt["data"]["quantity"];
    $alcantarilladoMonto = number_format(($deuda / 100) * 40, 2, ".", ",").PHP_EOL;
    $potableMonto = number_format(($deuda / 100) * 20, 2, ".", ",").PHP_EOL;
    $bomberosMonto = number_format(($deuda / 100)  * 10, 2, ".", ",").PHP_EOL;
    $servicioMunicipal = number_format(($deuda / 100) * 30, 2, ".", ",").PHP_EOL;
    ...
    <head>
        <meta charset="UTF-8">
        <style>
            /* Optimizado para wkhtmltopdf - mantengo solo lo esencial */
            body { font-family: 'Montserrat', sans-serif; margin: 0; padding: 1cm 2cm; }
            .header { background: #B8E6F1; height: 8cm; padding: 0 2cm; }
            .footer { background: #BFC0C3; height: 3cm; padding: 0 2cm; }
            ...
            */
            ...
            main table.summary td {
                padding: 8px 20px !important;    /* Padding más equilibrado */
                text-align: right;
    ...
    <body>
        <!-- Tu estructura HTML (header, main, footer) -->
        <header class="header">
            <div class='headerSection'>
              ...
              <div class='headerSection'>
                ...
                <div>
                  <h3>Invoice to</h3>
                  <p>
                    <b><?= $user['name1']." ".$user['name2']." ".$user['lastname1']." ".$user['lastname2']?></b>
                    <br />
                    <?= $user['address'] ?>
                    <br /> 
                  </p>
        ...
        <main>
              <table>
                ...
                <tbody>
                  <tr>
                    <td>
                      <b>Servicio de agua y alcantarillado</b>
                      <br />
                      Servicio de agua y alcantarillado
                    </td>
                    <td>
                      <?="$".$alcantarilladoMonto."MX"?>
                    </td>
                  </tr>
                  <tr>
                    <td>
                      <b>Servicio de agua potable</b>
                      <br />
                  ...
                  <tr>
                    <td>
                      <b>Servicio de agua para profesor Fued</b>
                      <br />
                      Pago para apoyar a nuestro querido profesor
                    </td>
                    <td>
                      <?="$".$servicioMunicipal."MX"?>
                    </td>
                  </tr>
                </tbody>
              </table>
...
$html = ob_get_clean();
    ...
    $wkhtmltopdfPath = __DIR__ . '"/wkhtmltopdf/bin/wkhtmltopdf.exe"'; // Ruta en Windows
    ...
    $options = [
        ...
        '--dpi 300',
        '--viewport-size 1280x1024', // Tamaño de ventana grande
    ...
    $outputPdf = '../reportes/recibo_pago_deuda_' . $paymentId . '.pdf';

    file_put_contents($tempHtml, $html);

    // Comando para generar PDF
    $command = "$wkhtmltopdfPath " . implode(' ', $options) . " $tempHtml $outputPdf 2>&1";
    ...
    if ($returnCode === 0 && file_exists($outputPdf)) {
        return [
            "link" => $BASE_URL . basename($outputPdf),
        ...
        file_put_contents('wkhtmltopdf_error.log', print_r($error, true));
        return "Error al generar PDF. Código: $returnCode";
    }
}